...
// Usage: node entropyVarianceNPM.js <runs>
// Example: node entropyVarianceNPM.js 100000

const entropy = require('../lib/entropy');
const stat = require('simple-statistics');

let runs = process.argv[2];
let results = [];
...
for (let i = 0; i < runs; i++) {
  let run = [];
  for (let j = 0; j < 8; j++) {
    run += 'abcdef1234567890'.charAt(Math.floor(Math.random() * 'abcdef1234567890'.length));
  }
  run +='-';
  for (let j = 0; j < 4; j++) {
    run += 'abcdef1234567890'.charAt(Math.floor(Math.random() * 'abcdef1234567890'.length));
